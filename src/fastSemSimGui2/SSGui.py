...
#
	...
	def OnSelectGO(self, event):
		...
		if self.SS_GO_CC_radio.GetValue():
			self.real_parent.SS_ontology = "CC"
		elif self.SS_GO_BP_radio.GetValue():
			self.real_parent.SS_ontology = "BP"
		elif self.SS_GO_MF_radio.GetValue():
			self.real_parent.SS_ontology = "MF"
		else:
			self.real_parent.SS_ontology = None
#
	...
	def OnSelectMS(self, event):
		...
		print c
		if c >= 0:
			self.real_parent.SS_mixing_strategy = self.SS_mix_box.GetString(self.SS_mix_box.GetCurrentSelection())
			...
			self.real_parent.SS_mixing_strategy = None
#
	...
	def OnSelectSS(self, event):
		...
		if c >= 0:
			self.real_parent.SS_measure = self.SS_measure_box.GetString(self.SS_measure_box.GetCurrentSelection())
			...
			if SemSimMeasures.SemSimMeasures[self.real_parent.SS_measure][1]:
			...
			self.real_parent.SS_measure = None
#
